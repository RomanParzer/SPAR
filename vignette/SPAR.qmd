  ...
  - name: Roman Parzer
    affiliations:
      - name: TU Wien
        ...
        postal-code: 1040
    orcid: 
    email: Roman.Parzer@tuwien.ac.at
...
-   `rp` an object of type [randomprojection]{.class}

-   `screencoef` an object of type [screencoef]{.class}
...
The objects for creating screening coefficients are implemented as \proglang{S}3 classes "`screencoef`". These objects are created by
several  `screen_*` functions, which take `...` and a list of controls `control`
as arguments. 
These functions return an object of class "`screencoef`"
which in turn is a list with three elements: 
  ...
  * `generate_fun` -- an [R]{.proglang} function for generating the screening coefficient. This
  function should have the following arguments:
    * a "`screencoef`" object which has as attributes all 
...
* `screen_corr()` -- computes the screening coefficients by the correlation between $y$ and $x_j$ using the function `stats::cor()`. It allows to pass a list of controls through the `control` argument to `stats::cor`.

* `screen_glmnet()` -- computes by default the ridge coefficient 
where the penalty $\lambda$ is very small [see @parzer2024glms for motivation]. The function 
...
`...`, which will be saved as attributes of the "`screencoef`" object.
...
object. 
In `generate_scrcoef_marglik`, we employ `family` attribute of the "`screencoef`" object if the control list argument does not contain any particular family.
...
spar_example_1 <- spar(
  ...
  family = binomial(),
  nummods = 100, 
  screencoef = screen_marglik(type = "fixed"),
