...
(GLMs) with high-dimensional (HD) predictors in \proglang{R}.
Probabilistic variable screening and
random projection of the predictors are performed to obtain an ensemble of GLMs, which are then averaged to obtain predictions in an HD setting.

\begin{itemize}
%%
\item Random projection  linearly maps a set of points in high dimensions into a much
lower-dimensional space while approximately preserving pairwise distances.
%%
\item Screening: For very large $p$, random projection can suffer from overfitting, as too many irrelevant predictors are being considered for prediction purposes % \cite{Dunson2020TargRandProj}
%%
\item Averaging is beneficial in practice to combine information of multiple such reductions \cite{Thanei2017RPforHDR}.
\end{itemize}

Several packages which provide functionality for random projections are available for \proglang{R}. Package \pkg{RandPro} \citep{RandProR} \citep{SIDDHARTH2020100629} allows for four different random projection matrices to be applied to the predictor matrix before employing one of $k$~nearest neighbor, support vector machine or naive Bayes classifier. Package \pkg{SPCAvRP} \citep{SPCAvRPR} implements sparse principal component analysis, based on the aggregation of eigenvector information from ``carefully-selected'' axis-aligned random projections of the sample covariance matrix.
...
Packages dealing with screening on CRAN:
\begin{itemize}
    \item Ball	Statistical Inference and Sure Independence Screening via Ball Statistics
    \item BayesS5	Bayesian Variable Selection Using Simplified Shotgun Stochastic Search with Screening (S5)
    \item bravo	Bayesian Screening and Variable Selection
    \item cdcsis	Conditional Distance Correlation Based Feature Screening and Conditional Independence Inference

    \item fusionclust	Clustering and Feature Screening using L1 Fusion Penalty
    \item LqG	Robust Group Variable Screening Based on Maximum Lq-Likelihood Estimation
    \item MFSIS	Moder-Free Sure Independent Screening Procedures
    \item QCSIS	Sure Independence Screening via Quantile Correlation and Composite Quantile Correlation
   \item https://cran.r-project.org/web/packages/RaSEn/index.html
    \item SIS \citep{SISR}
    \item SMLE	Joint Feature Screening via Sparse MLE
    \item tilting	: Variable Selection via Tilted Correlation Screening Algorithm
    \item TSGSIS	Two Stage-Grouped Sure Independence Screening
    \item VariableScreening:	High-Dimensional Screening for Semiparametric Longitudinal Regression
\end{itemize}
