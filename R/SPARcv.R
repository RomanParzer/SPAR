...
predict.spar.cv <- function(object,
  ...
  if (is.null(coef)) {
    coef <- coef(object,opt_par,nummod,lambda)
  }
  if (avg_type=="link") {
    if (type=="link") {
...
#' @param x result of spar.cv function of class "spar.cv".
...
plot.spar.cv <- function(x,
  ...
  if (plot_type=="res-vs-fitted") {
    ...
    if (plot_along=="lambda") {
      ...
      tmp_df <- subset(my_val_sum,lam==lambda)
      ind_min <- which.min(tmp_df$Meas)
...
print.spar.cv <- function(x, ...) {
  ...
  cat(sprintf("SPAR.cv object:\nSmallest CV-Meas %.1f reached for nummod=%d,
              ...
              formatC(mycoef_best$lambda,digits = 2,format = "e"),
